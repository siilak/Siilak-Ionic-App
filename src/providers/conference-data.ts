...
export class ConferenceData {
  ...
  load() {
    ...
    return new Promise(resolve => {
      ...
      this.http.get('assets/data/data.json').subscribe(res => {
        // we've got back the raw data, now generate the core myposts data
  ...
  processData(data) {
    ...
    // build up the data by linking team to sessions

    data.tracks = [];

    // loop through each day in the myposts
    data.myposts.forEach(day => {
  ...
  processSession(data, session) {
    ...
    if (session.speakerNames) {
      session.speakerNames.forEach(speakerName => {
        let speaker = data.team.find(s => s.name === speakerName);
        if (speaker) {
          session.team.push(speaker);
